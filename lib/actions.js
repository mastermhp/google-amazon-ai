...
// // Add this helper function for timeout handling
...
// // Update the generateContentWithGemini function
// async function generateContentWithGemini(data) {
//   const {
//     keyword,
//     keywords,
...
//   // Simplify image prompts to reduce prompt size
//   const imagePrompts = imageResults
//     ? imageResults
...
//   // Simplify prompt to reduce size
//   const prompt = promptTemplate.template
//     .replace("{{keyword}}", keyword)
//     .replace("{{keywords}}", keywords ? keywords.slice(0, 5).join(", ") : keyword)
//     .replace("{{titles}}", titles ? titles.slice(0, 3).join("\n") : "") // Reduced from 5 to 3
//     .replace("{{snippets}}", snippets ? snippets.slice(0, 3).join("\n") : "") // Reduced from 5 to 3
//     .replace("{{imagePrompts}}", imagePrompts)
//     .replace("{{contentType}}", contentType || "blog")
//     .replace("{{contentLength}}", contentLength || "medium")
...
//       "https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent", // Using a faster model
//       {
//         method: "POST",
//         headers: {
//           "Content-Type": "application/json",
...
//     // Generate fallback content if there's a timeout
//     if (error.name === "AbortError" || error.message.includes("timeout")) {
//       console.log("Generating fallback content due to timeout")
//       return generateFallbackContent(keyword, contentType)
//     }
...
// // Add this fallback content generator function
...
// // Update the searchGoogle function
// async function searchGoogle(keyword, country = "us") {
//   // First, search for text results
//   const textUrl = `https://www.googleapis.com/customsearch/v1?key=${process.env.GOOGLE_API_KEY}&cx=${process.env.GOOGLE_SEARCH_ENGINE_ID}&q=${encodeURIComponent(keyword)}&num=5&gl=${country}` // Reduced from 10 to 5

//   // Then, search for image results
//   const imageUrl = `https://www.googleapis.com/customsearch/v1?key=${process.env.GOOGLE_API_KEY}&cx=${process.env.GOOGLE_SEARCH_ENGINE_ID}&q=${encodeURIComponent(keyword)}&num=3&searchType=image&gl=${country}` // Reduced from 10 to 3
...
// Update the generateContentWithGemini function in lib/actions.js to properly handle image placeholders
async function generateContentWithGemini(data) {
  const {
    keyword,
    keywords,
  ...
  // Prepare image prompts to include in the content - make them more explicit
  const imagePrompts =
  ...
  // Configure content length based on selection
  const lengthConfig =
    {
      short: "500-800",
      medium: "1000-1500",
      long: "2000-3000",
    }[contentLength] || "1000-1500"
  ...
  // Add explicit instructions for word count and image placement
  const enhancedPrompt =
    prompt +
    `\n\nIMPORTANT: Make sure to include the image placeholders ([IMAGE1], [IMAGE2], etc.) at appropriate places in the content.\nIMPORTANT: The final word count should be at least ${lengthConfig} words.`
      ...
      "https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent",
      {
        ...
        body: JSON.stringify({
          ...
          generationConfig: {
            ...
            topK: 40,
            topP: 0.95,
            maxOutputTokens: contentLength === "long" ? 8192 : contentLength === "medium" ? 4096 : 2048,
          },
        }),
        agent: httpsAgent,
      },
      30000, // 30 second timeout for longer content
    )
