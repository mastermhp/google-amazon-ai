...
export default function AmazonContentGenerator({ products }) {
  ...
  const [contentLength, setContentLength] = useState("medium") // Added content length option
  ...
  const generateContent = async () => {
    ...
    try {
      ...
      const response = await fetch("/api/amazon-generate", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify({
          ...
          keywords,
          promptTemplateId: promptTemplateId !== "default" ? promptTemplateId : undefined,
          contentLength: contentLength, // Added content length
        }),
      ...
      await saveAmazonReviewAction({
        title,
        content: data.content || data.fallbackContent?.content || "",
        products: products.slice(0, 5), // Limit to 5 products
        keywords: keywords || "best products, product comparison, product review",
        seoMetrics: data.seoMetrics || data.fallbackContent?.seoMetrics,
        promptTemplateId: promptTemplateId !== "default" ? promptTemplateId : undefined,
      })
      ...
      // Create simple fallback content if there's an error
      const fallbackContent = {
        title: `Review of ${products.length} Products`,
        content: `# Product Review

This is a review of the selected products.
...
[PRODUCT_IMAGE${products.indexOf(p) + 1}]
...
`,
  ...
  .join("\n")}`,
      ...
      setContent(fallbackContent.content)
      setEditableContent(fallbackContent.content)
      setSeoMetrics(fallbackContent.seoMetrics)

      alert(`Error: ${error.message}. We've provided a simple template you can edit.`)
    } finally {
      setIsGenerating(false)
      setStatusMessage("")
    }
  ...
  const renderContent = (content) => {
    ...
    // Split content by image placeholders and render each part with images in between
    const parts = []
    const imagePlaceholderRegex = /\[PRODUCT_IMAGE(\d+)\]/gi
    let lastIndex = 0
    let match

    // Create a copy of the regex for the loop
    const regex = new RegExp(imagePlaceholderRegex)
    ...
    while ((match = regex.exec(processedContent)) !== null) {
      ...
      // Add the image
      const imageIndex = Number.parseInt(match[1]) - 1
      if (products && imageIndex >= 0 && imageIndex < products.length) {
        parts.push({
          type: "image",
    ...
    // If no image placeholders were found, just render the whole content
    if (parts.length === 0) {
      return <ReactMarkdown components={renderers}>{processedContent}</ReactMarkdown>
    }
          ...
          <React.Fragment key={`part-${index}`}>
            {part.type === "text" ? (
              ...
              <figure className="my-6">
                <img
                  src={part.product.image || "/placeholder.svg"}
                  alt={part.product.title}
                  className="w-full h-auto max-h-[300px] object-contain rounded-lg shadow-md mx-auto"
                />
                <figcaption className="text-sm text-gray-500 mt-2 text-center">{part.product.title}</figcaption>
              </figure>
            )}
          </React.Fragment>
        ))}
      ...
      <Card className="shadow-md">
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="space-y-2">
              <label htmlFor="contentLength" className="text-sm font-medium text-gray-700">
                Content Length
              </label>
              <Select value={contentLength} onValueChange={setContentLength} disabled={isGenerating}>
                <SelectTrigger>
